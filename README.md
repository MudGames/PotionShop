...
- 3개 이상 같은 재료가 나란히 놓이면 매치가 성립합니다. 매치에 실패하면 원위치로 돌아가며 이동
  횟수는 소모되지 않습니다
- 화면 옆 미션 패널에 표시된 재료를 이동 횟수(기본 20회) 안에 다 모으면 스테이지를 클리어하고 다음
...
> 다만 대기 상태에서도 프레임당 약 1KB 내외의 할당이 꾸준히 잡히는데, 이 잔여분의 정확한 발생
> 지점(타일/캐스케이드 코드 바깥의 어딘가 — DOTween 내부 갱신, UI/TextMeshPro 갱신, 혹은 에디터
> Play Mode 자체의 오버헤드일 가능성)은 아직 특정하지 못했습니다. **"1분 플레이 GC Alloc 0"은
> 캐스케이드 자체에 한해서는 사실상 달성됐고, 전체 0 달성은 이 잔여 원인을 더 조사해야 확정할 수
> 있습니다.** 또한 이 수치는 WebGL 빌드가 아니라 에디터 Play Mode에서 잰 것이라, 실제 배포 빌드의
> 수치와는 차이가 있을 수 있습니다.

### 주문(Order)을 매번 무작위로 생성하는 이유
처음에는 `LevelData`에 주문 재료/개수를 고정값으로 넣어뒀는데, 그러면 같은 스테이지를 다시 플레이할
때마다 항상 똑같은 미션이 나와 반복 플레이의 재미가 떨어졌습니다. `GridController` 생성자에서 재료
