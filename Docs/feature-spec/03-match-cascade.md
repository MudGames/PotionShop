...
1. 매치된 타일 제거 (풀로 반환) — 그냥 페이드아웃이 아니라 살짝 부풀었다가(팝) 줄어들며, 동시에
   ...
   같은 타이밍에 각 제거된 칸 위치에서 짧게 번쩍이는 버스트 이펙트(`BurstEffectView`/
   `BurstEffectPool`, 2026-08-05 추가, "터지는 연출이 필요합니다" 피드백)가 겹쳐 보인다 - 꽉 찬
   원형 그라디언트 스프라이트(`MatchBurst.png`)가 작게 시작해 커지며 빠르게 페이드아웃되고,
   팝 크기와 같은 비율로 커져서 큰 매치일수록 버스트도 커진다. 지속시간은 팝/페이드보다 일부러
   1.5배 길게 잡아(`BoardView.AnimateClear`) 반짝임이 눈에 띌 시간을 준다. 제자리에서 커지기만
   하는 플래시 하나만 있을 땐 "너무 정적입니다"는 피드백을 받아, 같은 위치에서 사방(6방향 +
   무작위 각도 보정)으로 흩날리며 작아지고 페이드아웃되는 작은 파편(`BurstShardView`/
   `BurstShardPool`, 같은 `MatchBurst.png`를 작게 재사용)을 추가해 실제로 터져서 흩어지는
   움직임을 더했다. 특수 타일(물약)로 승격되는 칸(anchor)도 이 팝+버스트를 똑같이 겪은 뒤 물약으로
   바뀐다(`TileView.AnimatePromoteToSpecial`, 2026-08-05 버그 픽스 - 이전에는 이 칸만 여기서 빠져
   있어 팝/버스트 없이 즉시 바뀌어 보였다. 파편은 타일이 실제로 사라지는 게 아니라서 생략,
   `12-special-tiles.md` §비주얼 참고).
...
7. **캐스케이드가 완전히 끝난 뒤(라운드가 계속되는 경우에 한해) 데드락(교환 가능한 매치가 하나도
   ...
   배치도 동일하게 검사한다(§`02-board-tile.md`). 이 안내 문구는 Complete/GameOver 배너와 같은
   방식으로 패널 전체를 어둡게 덮고 큰 글씨로 표시한다(2026-08-05, 보드 위쪽 얇은 띠만 덮던
   기존 방식은 글씨가 잘 안 보인다는 피드백으로 변경) — 게임을 멈추는 배너는 아니라 버튼은
   없고, 계속 자동으로 사라진다. 화면 전체를 덮는 배경(Dim)과 문구 텍스트는 `raycastTarget = false`
   로 둔다(2026-08-05 버그 픽스) — 위 문단처럼 재셔플 직후 입력은 바로 재활성화되는데,
   기본값(true)으로 두면 아직 표시 중인 이 오버레이가 그 입력(타일 클릭/드래그)을 가로채 버린다.
   `PuzzleHud.CreateComboPopup`이 같은 이유로 이미 이렇게 처리돼 있던 선례를 따름.
