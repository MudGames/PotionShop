...
namespace Puzzle.Core
{
    ...
    //  - run 길이 >= 5            -> run의 가운데 셀이 RadiusBomb으로 살아남는다(2026-08-04,
    //                                   기존 ColorBomb 대체 - "긴 매치 = 더 강한 보상" 의도 유지,
    //                                   ColorBomb은 활성화 시 배지가 재료 아이콘을 가려버려 어떤
    //                                   재료를 기억하는지 알 수 없다는 문제로 제거함).
    ...
    //                                   동시에 교차의 일부일 수 있기 때문이다). 5+ 규칙과 결과가
    //                                   같은 RadiusBomb이라 서로 충돌하지 않는다.
    public static class SpecialTileResolver
    {
        public static SpecialSpawnPlan Plan(IReadOnlyList<MatchRun> runs)
        {
            ...
            foreach (MatchRun run in runs)
            {
                ...
                {
                    GridCell anchor = run.Cells[run.Cells.Count / 2];
                    anchors[anchor] = SpecialKind.RadiusBomb;
                }
            }
        ...
        // 효과로 결합된다 (전형적인 매치3 UX). SpecialKind에는 두 가지 "계열"(line 계열, radius
        // bomb)이 있으므로 그 조합이 다뤄진다:
